...
def getListChapterReader(iduser):
    thong_tin = {}
    list_thong_tin = []
    config = {
        'user': 'leooRealman',
    ...
    try:
        ...
        mycursor.execute(f"SELECT * from recent_chapter_readed where id_user={iduser}")
        result_id_readed = mycursor.fetchall()
        print(result_id_readed)
        mycursor.execute(f"SELECT count(*) from recent_chapter_readed where id_user={iduser}")
